...
if [ $container -eq '' ]
then
    message="${now}${newline}All${space}build${space}result=${response}${newline}python"
else
    message="${now}${newline}${container}${space}build${space}result=${response}${newline}python"
fi
